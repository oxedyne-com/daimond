//! Daimond WS handler — chat protocol over WebSocket.
//!
...
/// Per-vhost configuration for Daimond, parsed from Steel's config.jdat.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DaimondConfig {
    pub llm_host:       String,
    pub llm_port:       u16,
    pub llm_path:       String,
    pub llm_key:        String,
...
impl DaimondConfig {
    ...
    pub fn from_datmap(m: &DaticleMap) -> Outcome<Self> {
        ...
        let llm_key = match m.get(&dat!("llm_key")) {
            Some(Dat::Str(s)) => s.clone(),
            _ => return Err(err!("DaimondConfig: 'llm_key' is required."; Invalid, Input, Missing)),
        };
        ...
        let system_prompt = match m.get(&dat!("system_prompt")) {
            Some(Dat::Str(s)) => s.clone(),
            _ => "You are Daimond, an AI assistant.".to_string(),
        };
...
/// State for the Daimond WS handler, shared across connections on a vhost.
#[derive(Clone)]
pub struct DaimondState<
    const UIDL: usize,
...
>(
    mut stream:     S,
    state:          DaimondState<UIDL, UID, ENC, KH, DB>,
    sid:            Option<String>,
    ...
    -> Outcome<()>
{
    ...
    match ws.connect_as_server(request).await {
        Ok(()) => {
            debug!("{}: Daimond WS handshake completed.", id);
        }
        Err(e) => return Err(err!(e,
            "{}: Daimond WS handshake failed.", id;
            IO, Network, Wire)),
    }

    // ── Build Daimond's own syntax ────────────────────────────────
    let syntax = match crate::syntax::build_syntax() {
        Ok(s) => s,
        Err(e) => {
            error!(e, "{}: Daimond: failed to build syntax.", id);
            return Ok(());
        }
    };
    ...
    let username = match get_username(&vhost_db, &sid) {
        Some(u) => {
             info!("{}: Daimond WS authenticated as '{}'.", id, u);
            u
        }
        None => {
             info!("{}: Daimond WS not authenticated (sid={:?}).", id, sid);
    ...
    loop {
        let msg = match ws.read().await {
            Ok(Some(WebSocketMessage::Text(txt))) => {
                debug!("{}: Daimond WS received text: '{}'", id, txt);
                txt
            }
            Ok(Some(WebSocketMessage::Binary(_))) => continue,
            Ok(Some(WebSocketMessage::Close(_, _))) => {
                info!("{}: Daimond WS client closed.", id);
                break;
            }
            Ok(None) => {
                info!("{}: Daimond WS client disconnected.", id);
                break;
            }
            Ok(_) => continue,
            Err(e) => {
                warn!("{}: Daimond WS read error: {}", id, e);
                break;
            }
        };
        ...
        debug!("{}: Daimond WS command '{}'.", id, cmd_name);
        ...
        match cmd_name.as_str() {
            ...
            "chat" => {
                ...
                // Expand any skill invocation (<name ...>) in the message by injecting the matching
                // skill's instructions -- and bound the turn by what that skill declared it needs.
                //
                // A skill is instructions, and instructions are as powerful as the agent reading
                // them. Its text is therefore not the thing to trust; its declaration is. A skill
                // that named `file_read` runs this turn against a registry holding `file_read` and
                // nothing else, so it cannot send mail, spawn an agent, or drive a logged-in
                // browser -- whatever its text says, and however cleverly it says it. The model is
                // not even shown the tools it did not ask for.
                //
                // A skill that declares nothing runs unrestricted, which is right for one the user
                // wrote themselves. When skills can be imported, an imported one that declares
                // nothing must be refused rather than trusted.
                let (content, registry) = match crate::skills::expand(&content, &registry.ctx.workspace) {
                    Ok(exp) => {
                        // A skill that ships a script and does not declare `shell` is refused, not
                        // narrowed: shipping code is asking to run it, and the user is owed the
                        // asking. Refused at expansion, so no turn runs at all.
                        if let Some(refusal) = &exp.refused {
                            let _ = ws.send(&text_msg(syntax.clone(), "error", vec![
                                dat!(refusal.clone()),
                            ])).await;
                            current_session = Some(session);
                        ...
                        match exp.declared_tools() {
                            None => (exp.text, registry),
                            Some(names) => {
                                let unknown = registry.unknown_tools(&names);
                                if !unknown.is_empty() {
                                    // A skill asking for a tool that does not exist has either a
                                    // typo or a misdescription of itself. Say so; do not quietly
                                    // hand it an empty toolbelt and let it fail obscurely.
                                    let _ = ws.send(&text_msg(syntax.clone(), "error", vec![
                                        dat!(fmt!(
                                            "That skill asks for {}, which {} not a tool Daimond \
                                            has. Fix its 'uses' line.",
                                            unknown.join(", "),
                                ...
                                let dirs = exp.skill_dirs();
                                let mut narrowed = registry.narrowed(&names);
                                // A bound is only a bound while the declaration cannot be edited.
                                // Skills live in the workspace, so a skill holding nothing but
                                // `file_write` could rewrite its own `uses` line -- or another
                                // skill's -- to ask for everything, and escape on the next
                                // invocation. One move, and the containment is theatre. So a turn
                                // running under a declaration is fenced out of Daimond's own
                                ...
                                info!(
                                    ...
                                    narrowed.tools.len(),
                                    registry.tools.len(),
                                    narrowed.tool_names().join(", "),
                                    if dirs.is_empty() {
                                        fmt!("nothing (it ships no files)")
                                    } else {
                                        dirs.join(", ")
                                    });
                                (exp.text, narrowed)
