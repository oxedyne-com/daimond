//! Daimond chat protocol syntax — command definitions for the
//! `/chat` WebSocket endpoint.
//!
//! Daimond owns its syntax independently of Steel.  Steel routes
//! WebSocket connections with path `/chat` to the Daimond handler;
//! Daimond then creates and uses its own `SyntaxRef` for parsing
//! incoming commands and constructing outgoing responses.
//!
//! Command categories:
//!
//! - **Daimond** — session management and chat (`session_new`, `chat`, …)
...
//! duplicated here so Daimond is self-contained and does not depend
...
/// Build the Daimond chat protocol syntax.
///
/// Defines all commands the Daimond WS handler can receive from the
/// client and all commands it can send back.  The version should
/// be bumped when the protocol changes incompatibly.
pub fn build_syntax()
    -> Outcome<SyntaxRef>
{
    let mut s = Syntax::new("daimond-chat")
        .ver(SemVer::new(0, 1, 0))
        .about("Daimond AI agent chat protocol");
