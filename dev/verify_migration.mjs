...
//
// The Red → Daimond rename moved two things a user's workspace actually holds: the per-Focus
// store (`foci/<id>/.red/` → `.daimond/`) and the standing-instructions file (`RED.md` →
// `DAIMOND.md`). Neither is Daimond's to lose. A Focus whose store is not found does not fail
...
// the way a user does, and then asks the questions a user would: is my Focus there, is its
...
// same OPFS sandbox the Focus store lives in — so these are the very bytes the old app wrote.
const seeded = await p.evaluate(async ({ id, rule }) => {
	...
	await w(`foci/${id}/.red/log`,
		'{"id":"r1","ts":1750000000000,"kind":"create","agent":"user","task":"create focus",'
		...
		+ `"delta_ref":"foci/${id}/.red/deltas/0001.md","note":"folded before the rename"}\n`);
...
await p.waitForTimeout(2500);      // loadFoci() and Instructions.refresh() run on unlock

// ── What the user sees ──────────────────────────────────────────────────
const listed = await p.$eval('#focus-list', e => e.textContent);
check('the Focus is still in the list', /An old pursuit/.test(listed), listed.trim().slice(0, 60));
...
const disk = await p.evaluate(async ({ id }) => {
	...
	return {
		newMeta:  await read(`foci/${id}/.daimond/meta.json`),
		newLog:   await read(`foci/${id}/.daimond/log`),
		newDelta: await read(`foci/${id}/.daimond/deltas/0001.md`),
		oldMeta:  await read(`foci/${id}/.red/meta.json`),
		oldLog:   await read(`foci/${id}/.red/log`),
		oldDelta: await read(`foci/${id}/.red/deltas/0001.md`),
		brief:    await read(`foci/${id}/brief.md`),
		version:  await read(`foci/${id}/versions/0001.md`),
...
check('the log’s delta_ref was rewritten to the new path',
	!!disk.newLog && disk.newLog.includes(`foci/${ID}/.daimond/deltas/0001.md`)
...
const again = await p.$eval('#focus-list', e => e.textContent);
const stillThere = await p.evaluate(async ({ id }) => {
	const mod = await import('../pkg/oxedyne_daimond.js');
	try { return await mod.read_file(`foci/${id}/.daimond/meta.json`); } catch { return null; }
}, { id: ID });
